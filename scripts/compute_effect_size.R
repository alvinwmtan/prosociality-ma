compute_effect_size <- function(participant_design, x_1 = NA, x_2 = NA, x_dif = NA,
  ...
  if (participant_design == "between") {
    ...
    if (is.na(corr) & metalabr:::complete(x_1, x_2, SD_1, SD_2, t)) {
      # Use raw means, SD, and t-values to calculate correlations
      corr = (SD_1^2 + SD_2^2 - (n_1 * (x_1 - x_2)^2 / t^2)) / (2 * SD_1 * SD_2)
      corr = max(min(corr, .99), -.99) # truncate to [-.99, .99]
    }
    if (is.na(corr) | corr > .99 | corr < -.99) {
    ...
    if (metalabr:::complete(n_1, d_calc)) {
      #now that effect size are calculated, effect size variance is calculated
    ...
    if (metalabr:::complete(x_1, x_2, SD_1)) {
      d_calc <- (x_1 - x_2) / SD_1
    ...
    } else if (metalabr:::complete(r, n_1)){
    ...
    } else  if (metalabr:::complete(d)) { # MLL added d_var_calc from data
      #if d and d_var were already reported, use those values
      d_calc <- d
      d_var_calc <- (1 / n_1) + (d_calc ^ 2 / (2 * n_1)) # this models what is done in metafor package, escalc(measure="SMCR"() (Viechtbauer, 2010)
      es_method  <- "d_one"
  ...
  if (!metalabr:::complete(d_calc, d_var_calc)) {
    ...
    if (metalabr:::complete(x_1, x_2, pooled_SD)) {
      d_calc <- (x_1 - x_2) / pooled_SD
    }
    if (metalabr:::complete(n_1, n_2, d_calc)) {
      d_var_calc <- ((n_1 + n_2) / (n_1 * n_2)) + (d_calc ^ 2 / (2 * (n_1 + n_2)))
    } else if (metalabr:::complete(n_1, d_calc)) {
      #d_var_calc <- (2/n_1) + (d_calc ^ 2 / (4 * n_1)) #we used this until 4/7/2017
      d_var_calc <- (1/n_1) + (d_calc ^ 2 / (2 * n_1))
    }
  }

  if (short_name == "prosocial" &&
      is.na(SD_1) && is.na(t) && is.na(d) &&
      metalabr:::complete(n_1, x_1, x_2) ) {
...
assignInNamespace("compute_effect_size", compute_effect_size, ns = "metalabr")
