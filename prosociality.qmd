...
options(ggplot2.discrete.colour = \() scale_colour_tableau(palette = "Superfishel Stone"),
        ggplot2.discrete.fill = \() scale_fill_tableau(palette = "Superfishel Stone"))
```
...
The replication included an additional `r df_rep[63:nrow(df_rep),] |> pull(long_cite) |> unique() |> setdiff(df_rep[1:62,] |> pull(long_cite) |> unique()) |> length()` papers with `r nrow(df_rep) - 62` effect sizes.
The looking time extension included an additional `r df_look |> pull(long_cite) |> unique() |> setdiff(df_rep |> pull(long_cite) |> unique()) |> length()` papers and an additional `r nrow(df_look)` effect sizes.
The neutral and other effect extension included an additional `r df_ps |> pull(long_cite) |> unique() |> setdiff(df_opp |> pull(long_cite) |> unique()) |> length()` papers and an additional `r nrow(df_ps) - nrow(df_opp)` effect sizes.
In total, this meta-analysis included `r df_ps |> pull(long_cite) |> unique() |> length()` papers and `r nrow(df_ps)` effect sizes.
...
```{r}
ggplot(df_rep, 
       aes(y = d_calc, x = Hamlin_Lab)) +
  geom_violin(aes(fill = Hamlin_Lab),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(labels = c("Other", "Hamlin")) +
  labs(x = "Lab of origin",
       y = "Cohen's d") +
...
```{r}
ggplot(df_rep, 
       aes(y = d_calc, x = mean_age_months)) +
  geom_point() +
  geom_smooth(col = "#6388B4",
              method = "lm") +
  labs(x = "Mean age (m)",
       y = "Cohen's d") +
  theme(legend.position = "none")
...
```{r}
ggplot(df_rep, 
       aes(y = d_calc, x = prop_excl)) +
  geom_point() +
  geom_smooth(col = "#6388B4",
              method = "lm") +
  labs(x = "Proportion of excluded infants",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
```{r}
ggplot(df_rep, 
       aes(y = d_calc, x = expt_condition)) +
  geom_violin(aes(fill = expt_condition),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(limits = c("help_hinder", "fair_unfair", "give_take", "other"),
                   labels = c("Help/hinder", "Fair/unfair", "Give/take", "Other")) +
  labs(x = "Scenario type",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
```{r}
p_rep_method <- ggplot(df_rep, 
       aes(y = d_calc, x = method)) +
  geom_violin(aes(fill = method),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(labels = c("Helping", "Reaching")) +
  labs(x = "Dependent variable",
       y = "Cohen's d") +
  theme(legend.position = "none")
p_rep_stimuli <- ggplot(df_rep, 
       aes(y = d_calc, x = stimuli_type)) +
  geom_violin(aes(fill = stimuli_type),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(labels = c("Cartoon", "Real")) +
  labs(x = "Stimuli",
       y = "Cohen's d") +
  theme(legend.position = "none")
p_rep_choice <- ggplot(df_rep, 
       aes(y = d_calc, x = choice_object)) +
  geom_violin(aes(fill = choice_object),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(limits = c("shapes", "puppets", "experimenters", "people"),
                   labels = c("Shapes", "Puppets", "Experimenters", "Cartoon people")) +
  labs(x = "Choice object",
       y = "Cohen's d") +
  theme(legend.position = "none")
(p_rep_method + p_rep_stimuli) / p_rep_choice
```

```{r}
ggplot(df_rep, 
       aes(y = d_calc, x = stimuli)) +
  geom_violin(aes(fill = stimuli),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(labels = c("Live show", "Movies")) +
  labs(x = "Presentation modality",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
p_look_present <- ggplot(df_look, 
  ...
  labs(x = "Presentation modality",
       y = "Cohen's d") +
  theme(legend.position = "none")
p_look_choice <- ggplot(df_look, 
       aes(y = d_calc, x = choice_object)) +
  geom_violin(aes(fill = choice_object),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(limits = c("shapes", "puppets", "experimenters", "people", "animals"),
                   labels = c("Shapes", "Puppets", "Experimenters", "Cartoon people", "Cartoon animals")) +
  labs(x = "Choice object",
       y = "Cohen's d") +
  theme(legend.position = "none")
(p_look_stimuli + p_look_present) / p_look_choice
```
...
ggplot(df_look, 
  ...
  geom_violin(aes(fill = method),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(limits = c("violation of expectation", "preferential looking", "anticipatory looking"),
                   labels = c("Violation of expectation", "Preferential looking", "Anticipatory looking")) +
  labs(x = "Method of measurement",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
This resulted in a significant difference between violation of expectation and preferential looking, $\beta =$ `r  ma_look_method_noantic$beta[2] |> abs() |> apa_num()`, $p=$ `r ma_look_method_noantic$QMp |> apa_p()`.
...
```{r}
ggplot(df_look, 
       aes(y = d_calc, x = target)) +
  geom_violin(aes(fill = target),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(limits = c("event itself", "3rd party approach", "3rd party reward", "preference"),
                   labels = c("Event itself", "Third party approach", "Third party reward", "Preference")) +
  labs(x = "Looking target",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
This did not explain any more variance in effect sizes, $Q_M$(3) $=$ `r ma_look_target$QM |> apa_num()`.

#### Participant design

```{r}
df_look_ppt <- df_look |> 
  mutate(participant_design = fct_recode(participant_design,
                                         within = "within_one",
                                         within = "within_two"))

ma_look_ppt <- rma.mv(d_calc ~ participant_design, 
                      V = d_var_calc, 
                      random = ~ 1 | short_cite/same_infant, 
                      data = df_look_ppt)
```
...
```{r}
ggplot(df_look_ppt, 
       aes(y = d_calc, x = participant_design)) +
  geom_violin(aes(fill = participant_design),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(labels = c("Between", "Within")) +
  labs(x = "Participant design",
       y = "Cohen's d") +
  theme(legend.position = "none")
```

The looking time studies also differed according to their participant design, with four studies (accounting for 10 effect sizes) using between-participant designs and 24 studies (accounting for 56 effect sizes) using within-participant designs.
There was a tendency towards within-participant designs having smaller effect sizes than between-participant designs, $\beta =$ `r ma_look_ppt$beta[2] |> apa_num()`, $Q_M$(1) $=$ `r ma_look_ppt$QM |> apa_num()`, $p=$ `r ma_look_ppt$QMp |> apa_p()`.
...
```{r}
ggplot(df_opp, 
       aes(y = d_calc, x = response_mode)) +
  geom_violin(aes(fill = response_mode),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(breaks = c("behavior", "eye-tracking"),
                   labels = c("Manual", "Looking time")) +
  labs(x = "Response mode",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
These studies also showed some patterns in similar directions to manual task studies, including the fact that age was not a significant moderator.
...
```{r}
ggplot(df_ps, 
       aes(y = d_calc, x = intent_val)) +
  geom_violin(aes(fill = intent_val),
              width = .7) +
  geom_boxplot(width = .2) +
  scale_x_discrete(limits = c("opp", "pos", "neg"),
                   labels = c("+/-", "+/0", "0/-")) +
  labs(x = "Intent valence",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
```{r}
ggplot(df_ps, 
       aes(y = d_calc, x = outcome_val)) +
  geom_violin(aes(fill = outcome_val)) +
  geom_boxplot(width = .2) +
  scale_x_discrete(limits = c("opp", "neut", "same", "rev"),
                   labels = c("+/-", "+/0, 0/-", "+/+, 0/0, -/-", "-/+")) +
  labs(x = "Outcome valence",
       y = "Cohen's d") +
  theme(legend.position = "none")
```
...
ggplot(data = df_cumul,
  ...
  scale_colour_tableau(labels = c("Hamlin", "Other"),
                       limits = c(TRUE, FALSE)) +
